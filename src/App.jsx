...
/* ─────────────── GLOBAL STYLES ─────────────── */
const G = `
  @import url('https://fonts.googleapis.com/css2?family=Press+Start+2P&family=DM+Sans:ital,wght@0,400;0,500;0,600;1,400&family=DM+Mono:wght@400;500&display=swap');
  ...
  @keyframes avatarGlow   { 0%,100%{filter:drop-shadow(0 0 8px rgba(124,58,237,0.5))} 50%{filter:drop-shadow(0 0 18px rgba(124,58,237,0.9)) drop-shadow(0 0 30px rgba(6,182,212,0.4))} }
  ...
  .toast { background:var(--paper); border:3px solid #7C3AED; border-radius:0; padding:0.75rem 1.1rem; font-size:0.9rem; font-family:'DM Mono', monospace; color:var(--ink); box-shadow:4px 4px 0 #7C3AED; display:flex; align-items:center; gap:0.6rem; pointer-events:all; animation:toastIn 0.35s ease both; }

  .float-cta { position:fixed; bottom:6rem; left:2rem; z-index:799; animation:floatCTA 0.5s ease both; }

  .kbd-hint { position:fixed; bottom:2rem; left:50%; transform:translateX(-50%); z-index:799; background:var(--paper); border:3px solid #7C3AED; border-radius:0; padding:0.45rem 1.2rem; font-size:0.82rem; font-family:'DM Mono', monospace; color:var(--muted); box-shadow:3px 3px 0 #7C3AED; display:flex; align-items:center; gap:0.6rem; animation:kbdIn 0.3s ease both; pointer-events:none; }
  .kbd-key { background:var(--vPale); color:#7C3AED; border:2px solid #7C3AED; border-radius:0; padding:0.1rem 0.45rem; font-size:0.8rem; font-weight:600; box-shadow:2px 2px 0 #7C3AED; }
    ...
    <div style={{
      ...
      <div style={{ position:"relative", zIndex:1, width:"min(480px,90vw)" }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"0.5rem" }}>
          ...
          <span style={{ fontSize:"0.75rem", color:"#7C3AED", fontWeight:600, fontFamily:"'DM Mono', monospace" }}>{progress}%</span>
        </div>
...
function TerminalEasterEgg({ onClose }) {
  ...
  return (
    <div onClick={e=>{ if(e.target===e.currentTarget){setVisible(false);setTimeout(onClose,380);}}} style={{ position:"fixed", inset:0, zIndex:9990, background:"rgba(0,0,0,0.85)", display:"flex", alignItems:"center", justifyContent:"center", padding:"1rem" }}>
      <div style={{ width:"min(700px,96vw)", maxHeight:"80vh", background:"#000", border:"4px solid #7C3AED", boxShadow:"8px 8px 0 #7C3AED", overflow:"hidden", display:"flex", flexDirection:"column", animation: visible?"terminalSlideIn 0.35s ease both":"terminalSlideOut 0.32s ease both", fontFamily:"'DM Mono', monospace" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0.7rem 1rem", background:"linear-gradient(135deg,#7C3AED,#5B21B6)", flexShrink:0 }}>
          <div style={{ display:"flex", alignItems:"center", gap:"0.5rem" }}>
            ...
            <span style={{ fontSize:"0.88rem", color:"rgba(255,255,255,0.9)", marginLeft:"0.5rem", fontFamily:"'Press Start 2P', monospace" }}>TERMINAL.EXE</span>
          </div>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"0.5rem", padding:"0.6rem 1.2rem", borderTop:"2px solid #7C3AED", background:"rgba(124,58,237,0.05)", flexShrink:0 }}>
          <span style={{ color:"#7C3AED", fontSize:"0.82rem", userSelect:"none" }}>ARMAN@PORTFOLIO</span>
          <span style={{ color:"rgba(255,255,255,0.3)", fontSize:"0.82rem", userSelect:"none" }}>:~$</span>
          <input ref={inputRef} value={input} onChange={e=>setInput(e.target.value)} onKeyDown={handleKey} autoComplete="off" spellCheck={false} placeholder="type command..." style={{ flex:1, background:"none", border:"none", outline:"none", fontSize:"0.9rem", color:"rgba(255,255,255,0.88)", caretColor:"#7C3AED", fontFamily:"'DM Mono', monospace" }}/>
  ...
  return (
    <div className="float-cta">
      <div style={{ background:"var(--paper)", border:"3px solid #7C3AED", boxShadow:"5px 5px 0 #7C3AED", padding:"1rem 1.2rem", maxWidth:220, position:"relative" }}>
        <button onClick={()=>setDismissed(true)} style={{ position:"absolute", top:"0.3rem", right:"0.4rem", background:"none", border:"2px solid #7C3AED", cursor:"pointer", fontSize:"0.82rem", color:"var(--muted)", width:18, height:18, display:"flex", alignItems:"center", justifyContent:"center" }}>✕</button>
        ...
        <div style={{fontSize:"0.82rem",fontWeight:600,color:"var(--ink)",marginBottom:"0.3rem",fontFamily:"'Press Start 2P', monospace",lineHeight:1.6}}>OPEN TO WORK</div>
        <div style={{fontSize:"0.88rem",color:"var(--muted)",marginBottom:"0.8rem",lineHeight:1.5}}>Backend · AI/ML · Full Stack</div>
        <a href="mailto:armanphaugat20@gmail.com" style={{ display:"block", textAlign:"center", padding:"0.5rem", background:"#7C3AED", color:"#fff", border:"none", fontSize:"0.85rem", fontWeight:600, textDecoration:"none", fontFamily:"'DM Sans', system-ui, sans-serif", boxShadow:"3px 3px 0 #4C1D95" }} onClick={()=>setDismissed(true)}>Let's talk →</a>
      </div>
    </div>
  );
}
...
function GitHubStatsCard() {
  ...
  return (
    <div style={{ background:"linear-gradient(135deg,#050010 0%,#0a0020 100%)", border:"3px solid #7C3AED", boxShadow:"8px 8px 0 #7C3AED, 0 0 20px rgba(124,58,237,0.15)", padding:"1.5rem", fontFamily:"'DM Sans', system-ui, sans-serif" }}>
  ...
  return (
    <div style={{background:"var(--paper)",border:"3px solid #7C3AED",padding:"1.5rem",boxShadow:"5px 5px 0 #7C3AED"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:"0.7rem",marginBottom:"1rem"}}>
        ...
        <div>
          <div style={{fontSize:"0.75rem",fontWeight:700,color:"var(--ink)",fontFamily:"'Press Start 2P', monospace"}}>AVAILABILITY</div>
        ...
        {days.map(d => (
          <div key={d} style={{textAlign:"center"}}>
            ...
            {slots[d].length>0?slots[d].map(t=>(
              <div key={t} style={{fontSize:"0.78rem",padding:"0.2rem",marginBottom:"0.2rem",background:"rgba(124,58,237,0.1)",color:"#7C3AED",fontFamily:"'DM Mono', monospace",border:"1px solid #7C3AED",fontWeight:500}}>{t}</div>
            )):(
  ...
  return (
    <div style={{display:"flex",justifyContent:"center",alignItems:"center",flexDirection:"column"}}>
      <svg width="320" height="320" viewBox="0 0 320 320">
        ...
        {skills.map((s,i)=>{const p=polarToXY(i,(s.val/100)*R-14);return <text key={`v${i}`} x={p.x} y={p.y} textAnchor="middle" dominantBaseline="middle" fill="#7C3AED" fontSize="9" fontFamily="'DM Mono', monospace" fontWeight="500">{s.val}%</text>;})}
      </svg>
    </div>
  );
}
...
function Cursor() {
  ...
  return (
    <>
      <div ref={dot} style={{position:"fixed",width:8,height:8,background:"#7C3AED",transform:"translate(-50%,-50%)",pointerEvents:"none",zIndex:9999}}/>
      <div ref={trail} style={{position:"fixed",width:20,height:20,border:"2px solid #7C3AED",transform:"translate(-50%,-50%)",pointerEvents:"none",zIndex:9998,opacity:.5}}/>
    </>
  );
}
  ...
  return (
    <>
      <div className={`mob-menu${menuOpen?" open":""}`}>
        ...
        <button onClick={()=>setDark(d=>!d)} style={{ background:"none", border:"3px solid #7C3AED", padding:"0.6rem 1.4rem", cursor:"pointer", fontSize:"0.9rem", display:"flex", alignItems:"center", gap:"0.5rem", fontFamily:"'DM Sans', system-ui, sans-serif", fontWeight:600, color:"var(--ink)", boxShadow:"3px 3px 0 #7C3AED" }}>
      ...
      <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:600, display:"flex", alignItems:"center", justifyContent:"space-between", padding:scrolled||menuOpen?"0.8rem 2rem":"1.2rem 2rem", background:scrolled||menuOpen?"var(--bg)":"transparent", borderBottom:scrolled?"3px solid #7C3AED":"3px solid transparent", boxShadow:scrolled?"0 3px 0 #7C3AED":"none", transition:"all 0.25s ease", fontFamily:"'DM Sans', system-ui, sans-serif" }}>
        <div style={{display:"flex",alignItems:"center",gap:"0.7rem"}}>
          <div style={{width:32,height:32,background:"#7C3AED",border:"2px solid #4C1D95",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"3px 3px 0 #4C1D95"}}>
            <span style={{color:"#fff",fontSize:"0.82rem",fontWeight:700,fontFamily:"'Press Start 2P', monospace"}}>AP</span>
          </div>
          <span style={{fontSize:"0.95rem",color:"#7C3AED",fontWeight:600}}>arman<span style={{color:C.muted}}>.exe</span></span>
        </div>
        ...
        <div className="hide-mob" style={{display:"flex",gap:"0.5rem",alignItems:"center"}}>
          ...
          <a href="mailto:armanphaugat20@gmail.com" style={{ padding:"0.55rem 1.1rem", background:"#7C3AED", color:"#fff", border:"2px solid #4C1D95", fontSize:"0.92rem", fontWeight:600, textDecoration:"none", boxShadow:"3px 3px 0 #4C1D95", transition:"all 0.1s", whiteSpace:"nowrap" }}
          ...
          >Hire me ▶</a>
          <a href="/ARMANRESUME.pdf" download="ARMANRESUME.pdf" style={{ padding:"0.55rem 1.1rem", background:"transparent", color:"#7C3AED", border:"2px solid #7C3AED", fontSize:"0.92rem", fontWeight:600, textDecoration:"none", display:"flex", alignItems:"center", gap:"0.4rem", animation:"resumePulse 2.5s ease-in-out infinite", boxShadow:"3px 3px 0 #7C3AED", whiteSpace:"nowrap" }}
            onMouseEnter={e=>{e.currentTarget.style.background="#7C3AED";e.currentTarget.style.color="#fff";}}
    ...
    <div className="progress-sidebar" style={{ position:"fixed", right:"1.2rem", top:"50%", transform:"translateY(-50%)", zIndex:700, display:"flex", flexDirection:"column", alignItems:"center", gap:"0.5rem" }}>
      {SECTIONS.map((id,i)=>(
        <div key={id} style={{position:"relative",display:"flex",alignItems:"center",gap:"0.4rem"}}>
          {active===i && (
            <div style={{ position:"absolute", right:"calc(100% + 0.5rem)", background:"var(--paper)", border:"2px solid #7C3AED", borderRadius:0, padding:"0.15rem 0.5rem", fontSize:"0.8rem", fontFamily:"'DM Sans', system-ui, sans-serif", color:"#7C3AED", whiteSpace:"nowrap", fontWeight:600, animation:"backTopIn 0.2s ease both", boxShadow:"2px 2px 0 #7C3AED" }}>{SECTION_LABELS[i]}</div>
          )}
...
function PixelRain() {
  ...
  useEffect(() => {
    ...
    resize();
    window.addEventListener("resize", resize);
    const COLS_COLORS = ["#7C3AED","#A78BFA","#06B6D4","#67E8F9","#10B981","#F59E0B"];
    ...
    const draw = () => {
      frame++;
      ctx.fillStyle = "rgba(6, 0, 15, 0.18)";
...
const ChipTune = (() => {
  ...
  let _muted = false;
  const play = (type, freq, duration, vol = 0.08) => {
    if (_muted) return;
    try {
  ...
  return {
    blip:   () => play("square", 880, 0.08, 0.06),
    click:  () => play("square", 440, 0.12, 0.07),
    select: () => {
      try {
        const ac=getCtx();
        [523,659,784].forEach((f,i)=>{
          ...
          osc.connect(gain); gain.connect(ac.destination);
          osc.type="square"; osc.frequency.setValueAtTime(f, ac.currentTime+i*0.07);
          gain.gain.setValueAtTime(0.06, ac.currentTime+i*0.07);
          gain.gain.exponentialRampToValueAtTime(0.001, ac.currentTime+i*0.07+0.14);
          osc.start(ac.currentTime+i*0.07); osc.stop(ac.currentTime+i*0.07+0.15);
        });
      } catch(e) {}
    },
    coin: () => {
      try {
        ...
        [988,1319].forEach((f,i)=>{
          const osc=ac.createOscillator(); const gain=ac.createGain();
          osc.connect(gain); gain.connect(ac.destination);
          osc.type="square"; osc.frequency.setValueAtTime(f, ac.currentTime+i*0.1);
          gain.gain.setValueAtTime(0.07, ac.currentTime+i*0.1);
          gain.gain.exponentialRampToValueAtTime(0.001, ac.currentTime+i*0.1+0.18);
          osc.start(ac.currentTime+i*0.1); osc.stop(ac.currentTime+i*0.1+0.19);
        });
      } catch(e) {}
    },
    buzz:   () => play("sawtooth", 160, 0.15, 0.06),
    tick:   () => play("sine", 660, 0.05, 0.04),
    setMuted: (v) => { _muted = v; },
  };
})();
...
function useChipTune() {
  useEffect(() => {
    const onEnter = (e) => {
      const el = e.target.closest("a,button,[data-chip]");
      if (!el) return;
      if (el.classList.contains("px-btn") || el.tagName === "BUTTON") ChipTune.blip();
      else if (el.tagName === "A") ChipTune.tick();
    };
    const onClick = (e) => {
      const el = e.target.closest("a,button,[data-chip]");
      if (!el) return;
      if (el.classList.contains("px-btn")) ChipTune.select();
      else ChipTune.click();
    };
...
function ChipTuneToggle() {
  const [muted, setMuted] = useState(false);
  return (
    <button onClick={() => { setMuted(m => { const next=!m; ChipTune.setMuted(next); if(!next)ChipTune.coin(); return next; }); }}
      title={muted ? "Enable sounds" : "Disable sounds"}
      style={{ width:34,height:34, background:muted?"var(--vPale)":"#7C3AED", border:`2px solid ${muted?"#7C3AED":"#4C1D95"}`, cursor:"pointer", display:"flex",alignItems:"center",justifyContent:"center", boxShadow:`2px 2px 0 ${muted?"#7C3AED":"#4C1D95"}`, transition:"all 0.1s", fontSize:"1rem", flexShrink:0 }}
      onMouseEnter={e=>e.currentTarget.style.transform="translate(-1px,-1px)"}
      onMouseLeave={e=>e.currentTarget.style.transform=""}
    >{muted?"🔇":"🔊"}</button>
  );
...
function Hero() {
  ...
  return (
    <section id="hero" className="hero-sec" style={{ minHeight:"100vh", display:"flex", flexDirection:"column", justifyContent:"center", padding:"9rem 4rem 5rem", position:"relative", overflow:"hidden", background:"var(--bg)", backgroundImage:"radial-gradient(ellipse 80% 50% at 50% -10%, rgba(124,58,237,0.1) 0%, transparent 70%)" }}>
      <div className="float-bg" style={{position:"absolute",inset:0,pointerEvents:"none",zIndex:0,overflow:"hidden"}}>
        <div style={{position:"absolute",inset:0,backgroundImage:"linear-gradient(rgba(124,58,237,0.05) 1px,transparent 1px),linear-gradient(90deg,rgba(124,58,237,0.05) 1px,transparent 1px)",backgroundSize:"48px 48px",zIndex:0}}/>
      ...
      <div style={{maxWidth:1060,margin:"0 auto",width:"100%",position:"relative",zIndex:1}}>
        <div className="rv" style={{display:"flex",alignItems:"center",gap:"1rem",marginBottom:"1.4rem",flexWrap:"wrap"}}>
          <div style={{fontFamily:"'DM Sans', system-ui, sans-serif",fontSize:"1rem",color:"#7C3AED",letterSpacing:"1px",display:"flex",alignItems:"center",gap:"0.8rem",fontWeight:500}}>
        ...
        <h1 className="rv d1" style={{fontFamily:"'Press Start 2P', monospace",fontSize:"clamp(2.2rem,5.5vw,5rem)",fontWeight:400,lineHeight:1.15,marginBottom:"0.9rem",letterSpacing:"4px",color:"var(--ink)",animation:"heroGlow 4s ease-in-out infinite"}}>
          ARMAN<br/>
        ...
        <p className="rv d3" style={{fontSize:"1.05rem",color:"var(--muted)",maxWidth:540,lineHeight:1.85,marginBottom:"2.5rem",fontFamily:"'DM Sans', system-ui, sans-serif",fontWeight:400}}>
          ...
          Top <strong style={{color:"#7C3AED",fontWeight:600}}>0.3% LeetCode</strong> · <strong style={{color:"#06B6D4",fontWeight:600}}>9.05 CGPA</strong> · <strong style={{color:"#F59E0B",fontWeight:600}}>900+ DSA</strong> solved.
        </p>
        ...
        <div className="rv d4 hero-ctarow" style={{display:"flex",gap:"0.8rem",flexWrap:"wrap",marginBottom:"3.5rem"}}>
          {[
            ...
            {href:"#contact",label:"Let's Talk",bg:"transparent",color:"var(--ink)",shadow:"#7C3AED",bord:"#7C3AED"},
          ...
          <a href="/ARMANRESUME.pdf" download="ARMANRESUME.pdf" className="px-btn" style={{ padding:"0.8rem 1.5rem", background:"transparent", color:"#7C3AED", border:"2px solid #7C3AED", fontSize:"0.92rem", fontWeight:600, textDecoration:"none", fontFamily:"'DM Sans', system-ui, sans-serif", boxShadow:"4px 4px 0 #7C3AED", display:"inline-flex", alignItems:"center", gap:"0.4rem", animation:"resumePulse 2.5s ease-in-out infinite" }}>
            <Download size={12} strokeWidth={2.5}/>Resume
          </a>
        </div>

        <div className="rv" style={{display:"flex",gap:"0",borderTop:"2px solid rgba(124,58,237,0.2)",paddingTop:"2rem",marginTop:"0.5rem",flexWrap:"wrap"}}>
      ...
      <div style={{display:"flex",animation:"marquee 25s linear infinite",width:"max-content"}}>
        {doubled.map((item,i)=>(
          <span key={i} style={{color:COLS[i%COLS.length],fontSize:"0.95rem",letterSpacing:"1px",padding:"0 1.8rem",display:"flex",alignItems:"center",gap:"1.5rem",fontFamily:"'DM Sans', system-ui, sans-serif",fontWeight:500}}>
            {item}<span style={{color:"rgba(124,58,237,0.6)",fontSize:"0.7rem"}}>■</span>
          </span>
        ))}
      </div>
    </div>
  ...
  return (
    <section id="about" style={{padding:"8rem 4rem",background:"var(--bg2)",position:"relative",overflow:"hidden"}} className="sec-pad">
      ...
      <div style={{maxWidth:1060,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr 1fr",gap:"4rem",alignItems:"start",position:"relative",zIndex:1}} className="grid2">
        <div>
          <div className="rv" style={{fontFamily:"'Press Start 2P', monospace",fontSize:"0.75rem",color:"#7C3AED",letterSpacing:"3px",textTransform:"uppercase",marginBottom:"1rem",display:"flex",alignItems:"center",gap:"0.8rem",fontWeight:400}}>
            <span style={{width:20,height:4,background:"#7C3AED",display:"inline-block",boxShadow:"2px 2px 0 #4C1D95"}}/>ABOUT ME
          </div>
          <h2 className="rv d1" style={{fontFamily:"'Press Start 2P', monospace",fontSize:"clamp(1.2rem,2.5vw,2rem)",fontWeight:400,lineHeight:1.4,marginBottom:"1.5rem",color:"var(--ink)"}}>
          ...
          <div className="rv d2" style={{fontSize:"1rem",color:"var(--muted)",lineHeight:2,fontFamily:"'DM Sans', system-ui, sans-serif",fontWeight:400}}>
            ...
            <p style={{marginBottom:"1rem"}}>Core focus: backend engineering — <strong style={{color:"#7C3AED",fontWeight:600}}>Node.js, Redis, BullMQ, MySQL</strong> — plus AI apps with <strong style={{color:"#06B6D4",fontWeight:600}}>LangChain, FAISS, and Groq</strong>. ML models with Scikit-learn and XGBoost.</p>
        ...
        <div ref={cardRef} className="rv d2 px-card" style={{background:"var(--paper)",padding:"2rem",willChange:"transform",transition:"transform 0.15s ease"}}>
          {[
            ...
            <div key={label} style={{display:"flex",gap:"0.9rem",alignItems:"flex-start",padding:"0.8rem 0",borderBottom:i<5?`1px solid rgba(124,58,237,0.08)`:"none"}}>
  ...
  return (
    <section id="experience" style={{padding:"8rem 4rem",background:"var(--bg)"}} className="sec-pad">
      <div style={{maxWidth:1060,margin:"0 auto"}}>
        <div className="rv" style={{fontFamily:"'Press Start 2P', monospace",fontSize:"0.75rem",color:"#7C3AED",letterSpacing:"2px",marginBottom:"1rem",display:"flex",alignItems:"center",gap:"0.6rem"}}>
        ...
        <div className="rv d3 px-card" style={{background:"var(--paper)",padding:"2rem"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:"1rem",marginBottom:"1.2rem"}}>
            <div style={{display:"flex",gap:"1rem",alignItems:"center"}}>
              ...
              <div>
                ...
                <div style={{fontSize:"0.92rem",color:"#7C3AED",fontWeight:500,fontFamily:"'DM Sans', system-ui, sans-serif"}}>Indavis Lifesciences, Haridwar</div>
              </div>
            </div>
            <div style={{textAlign:"right"}}>
              <span style={{fontFamily:"'DM Mono', monospace",fontSize:"0.88rem",color:"#7C3AED",background:"var(--vPale)",padding:"0.3rem 0.8rem",display:"inline-block",border:"2px solid #7C3AED",boxShadow:"2px 2px 0 #7C3AED",fontWeight:500}}>Jun–Jul 2025</span>
          ...
          {["Maintained and updated the company website ensuring smooth performance and content accuracy.","Collaborated with cross-functional teams to align website updates with brand guidelines and business objectives."].map((b,i)=>(
            <div key={i} style={{display:"flex",gap:"0.6rem",fontSize:"0.97rem",color:"var(--muted)",marginBottom:"0.55rem",lineHeight:1.75,fontFamily:"'DM Sans', system-ui, sans-serif"}}>
              <span style={{color:"#7C3AED",flexShrink:0}}>▸</span>{b}
            </div>
          ))}
        ...
        <div className="rv d4 otw-banner" style={{marginTop:"1.5rem",background:"linear-gradient(135deg,#1a0a2e 0%,#0d0030 100%)",border:"3px solid #7C3AED",padding:"2rem",boxShadow:"8px 8px 0 #7C3AED",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:"1.5rem"}}>
          <div>
            ...
            <div style={{fontFamily:"'Press Start 2P', monospace",fontSize:"0.75rem",fontWeight:400,color:"#fff",marginBottom:"0.5rem",lineHeight:1.6}}>SUMMER INTERNSHIP 2026</div>
    ...
    <section id="resume" style={{padding:"8rem 4rem",background:"var(--bg)"}} className="sec-pad">
      <div style={{maxWidth:1060,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",flexWrap:"wrap",gap:"1.5rem",marginBottom:"3rem"}}>
          ...
          <a href="/ARMANRESUME.pdf" download="ARMANRESUME.pdf" className="rv d2 px-btn" style={{padding:"0.8rem 1.5rem",background:"#7C3AED",color:"#fff",border:"2px solid #4C1D95",textDecoration:"none",fontSize:"0.95rem",fontWeight:600,display:"flex",alignItems:"center",gap:"0.5rem",boxShadow:"4px 4px 0 #4C1D95",animation:"resumePulse 2.5s ease-in-out infinite",fontFamily:"'DM Sans', system-ui, sans-serif"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:"1.5rem"}}>
            <div className="rv px-card shimmer-hover" style={{background:"var(--paper)",padding:"1.5rem"}}>
              <div style={{display:"flex",alignItems:"center",gap:"0.6rem",marginBottom:"1rem"}}>
                ...
                <div style={{fontFamily:"'Press Start 2P', monospace",fontSize:"0.65rem",fontWeight:400,color:"var(--ink)"}}>SUMMARY</div>
              </div>
              <p style={{fontSize:"0.97rem",color:"var(--muted)",lineHeight:1.9,fontFamily:"'DM Sans', system-ui, sans-serif"}}>
                Passionate <strong style={{color:"var(--ink)",fontWeight:600}}>Backend & AI/ML Engineer</strong> in my 3rd year of B.Tech CSE at MUJ, maintaining a <strong style={{color:"#7C3AED",fontWeight:600}}>9.05 CGPA</strong> with the Dean's Excellence Award. Builds high-performance systems with <strong style={{color:"#7C3AED",fontWeight:600}}>Node.js, Redis, and MySQL</strong>. Ranked in the <strong style={{color:"#06B6D4",fontWeight:600}}>top 0.3% globally on LeetCode</strong> with 900+ problems solved.
              </p>
              <div style={{display:"flex",gap:"0.4rem",flexWrap:"wrap",marginTop:"1rem"}}>
                {["Backend","AI/ML","Open to Work"].map((t,i)=>(
                  <span key={t} style={{padding:"0.25rem 0.7rem",background:`${["#7C3AED","#06B6D4","#10B981"][i]}18`,border:`2px solid ${["#7C3AED","#06B6D4","#10B981"][i]}`,fontSize:"0.88rem",color:["#7C3AED","#06B6D4","#10B981"][i],fontWeight:600,fontFamily:"'DM Sans', system-ui, sans-serif"}}>{t}</span>
                ))}
              </div>
            </div>
            ...
            <div className="rv px-card shimmer-hover" style={{background:"var(--paper)",padding:"1.5rem"}}>
              <div style={{display:"flex",alignItems:"center",gap:"0.8rem",marginBottom:"1.2rem"}}>
                <div style={{width:38,height:38,background:"linear-gradient(135deg,#7C3AED,#06B6D4)",border:"2px solid #4C1D95",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"3px 3px 0 #4C1D95"}}>
                  <span style={{color:"#fff",fontSize:"0.65rem",fontWeight:700,fontFamily:"'Press Start 2P', monospace"}}>AP</span>
                </div>
                <div>
              ...
              {[
                ...
                <div key={label} style={{display:"flex",gap:"0.7rem",alignItems:"center",padding:"0.45rem 0",borderBottom:"1px solid rgba(124,58,237,0.06)"}}>
                  ...
                  {href?<a href={href} target={href.startsWith("http")?"_blank":undefined} rel="noreferrer" style={{fontSize:"0.92rem",color:"#7C3AED",textDecoration:"none",fontFamily:"'DM Sans', system-ui, sans-serif",wordBreak:"break-all",fontWeight:500}}>{label}</a>:<span style={{fontSize:"0.92rem",color:"var(--muted)",fontFamily:"'DM Sans', system-ui, sans-serif"}}>{label}</span>}
                </div>
              ))}
            </div>
            ...
            <div className="rv d1 px-card" style={{background:"var(--paper)",padding:"1.5rem"}}>
              <div style={{display:"flex",alignItems:"center",gap:"0.6rem",marginBottom:"1.2rem"}}>
                ...
                <div style={{fontFamily:"'Press Start 2P', monospace",fontSize:"0.62rem",fontWeight:400,color:"var(--ink)"}}>EDUCATION</div>
              </div>
              <div style={{borderLeft:"3px solid #7C3AED",paddingLeft:"1rem"}}>
                ...
                <div style={{fontSize:"0.95rem",color:"#7C3AED",fontWeight:500,marginTop:"0.2rem",fontFamily:"'DM Sans', system-ui, sans-serif"}}>Manipal University Jaipur</div>
                ...
                <div style={{marginTop:"0.6rem",display:"flex",gap:"0.4rem",flexWrap:"wrap"}}>
                  <span style={{padding:"0.3rem 0.75rem",background:"var(--vPale)",color:"#7C3AED",fontSize:"0.88rem",fontWeight:600,border:"2px solid #7C3AED",fontFamily:"'DM Mono', monospace",boxShadow:"2px 2px 0 #7C3AED"}}>9.05 CGPA</span>
            ...
            <div className="rv d2 px-card" style={{background:"var(--paper)",padding:"1.5rem"}}>
              <div style={{display:"flex",alignItems:"center",gap:"0.6rem",marginBottom:"1.2rem"}}>
                <div style={{width:28,height:28,background:"rgba(245,158,11,0.15)",border:"2px solid #F59E0B",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"0.85rem",boxShadow:"2px 2px 0 #F59E0B"}}>⭐</div>
                <div style={{fontFamily:"'Press Start 2P', monospace",fontSize:"0.62rem",fontWeight:400,color:"var(--ink)"}}>ACHIEVEMENTS</div>
              </div>
              {[
                ...
                {icon:"💡",text:"900+ DSA problems solved across platforms",col:"#06B6D4"},
              ].map(({icon,text,col})=>(
            ...
            <div className="rv d3 px-card" style={{background:"var(--paper)",padding:"1.5rem"}}>
              <div style={{display:"flex",alignItems:"center",gap:"0.6rem",marginBottom:"1.2rem"}}>
                ...
                <div style={{fontFamily:"'Press Start 2P', monospace",fontSize:"0.62rem",fontWeight:400,color:"var(--ink)"}}>CERTIFICATIONS</div>
              </div>
              {[
                ...
                <div key={name} style={{display:"flex",gap:"0.8rem",alignItems:"flex-start",padding:"0.8rem 0",borderBottom:i<5?"1px solid rgba(124,58,237,0.06)":"none"}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{display:"flex",flexWrap:"wrap",gap:"0.25rem",marginTop:"0.4rem"}}>
                      {skills.map(s=><span key={s} style={{fontSize:"0.85rem",padding:"0.12rem 0.4rem",background:"var(--vPale)",color:"#7C3AED",border:"1px solid #7C3AED",fontFamily:"'DM Sans', system-ui, sans-serif",fontWeight:500}}>{s}</span>)}
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:"1.5rem"}}>
            <div className="rv px-card shimmer-hover" style={{background:"var(--paper)",padding:"1.5rem"}}>
              <div style={{display:"flex",alignItems:"center",gap:"0.6rem",marginBottom:"1.2rem"}}>
                ...
                <div style={{fontFamily:"'Press Start 2P', monospace",fontSize:"0.62rem",fontWeight:400,color:"var(--ink)"}}>WORK EXPERIENCE</div>
              </div>
              <div style={{borderLeft:"3px solid #7C3AED",paddingLeft:"1rem"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:"0.4rem"}}>
                  <div>
                    ...
                    <div style={{fontSize:"0.92rem",color:"#7C3AED",fontWeight:500,marginTop:"0.1rem",fontFamily:"'DM Sans', system-ui, sans-serif"}}>Indavis Lifesciences, Haridwar</div>
                  </div>
                  <span style={{fontFamily:"'DM Mono', monospace",fontSize:"0.82rem",color:"#7C3AED",background:"var(--vPale)",padding:"0.2rem 0.5rem",border:"1px solid #7C3AED",fontWeight:500}}>Jun–Jul 2025</span>
                </div>
            ...
            <div className="rv d2 px-card" style={{background:"var(--paper)",padding:"1.5rem"}}>
              ...
              {[
                ...
                <div key={name} style={{padding:"0.8rem 0",borderBottom:i<3?"1px solid rgba(124,58,237,0.06)":"none"}}>
                  ...
                  <div style={{fontFamily:"'DM Mono', monospace",fontSize:"0.82rem",color:"#7C3AED",marginBottom:"0.4rem",fontWeight:500}}>{tech}</div>
                  {points.map((p,j)=>(
                    <div key={j} style={{display:"flex",gap:"0.4rem",fontSize:"0.92rem",color:"var(--muted)",lineHeight:1.6,marginBottom:"0.15rem",fontFamily:"'DM Sans', system-ui, sans-serif"}}>
                      <span style={{color:"#7C3AED",flexShrink:0}}>›</span>{p}
                    </div>
                  ))}
                </div>
              ))}
        ...
        <div className="rv d4" style={{marginTop:"2rem",background:"#1a0a2e",border:"3px solid #7C3AED",padding:"1.8rem 2rem",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:"1.5rem",boxShadow:"6px 6px 0 #7C3AED"}}>
          ...
          <div style={{display:"flex",gap:"0.8rem",flexWrap:"wrap"}}>
            <a href="/ARMANRESUME.pdf" download className="px-btn" style={{padding:"0.8rem 1.5rem",background:"#7C3AED",color:"#fff",border:"2px solid #A78BFA",textDecoration:"none",fontWeight:600,fontSize:"0.95rem",display:"flex",alignItems:"center",gap:"0.5rem",fontFamily:"'DM Sans', system-ui, sans-serif",boxShadow:"3px 3px 0 #A78BFA"}}>
    ...
    <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)} style={{ background:hov?"#1a0a2e":"var(--paper)", border:`3px solid ${cc}`, boxShadow:hov?`8px 8px 0 ${cc}`:`4px 4px 0 ${cc}`, padding:"1.6rem", transition:"all 0.1s", transform:hov?"translate(-4px,-4px)":"translate(0,0)", display:"flex", flexDirection:"column", cursor:"default" }}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"0.8rem"}}>
        <div style={{display:"flex",gap:"0.4rem",alignItems:"center"}}>
          ...
          <span style={{fontFamily:"'DM Sans', system-ui, sans-serif",fontSize:"0.85rem",fontWeight:600,padding:"0.2rem 0.6rem",background:`${cc}18`,color:cc,border:`1px solid ${cc}`}}>{p.category}</span>
        </div>
      ...
      <div style={{flex:1,marginBottom:"1rem"}}>
        {p.highlights.map((h,i)=>(
          <div key={i} style={{display:"flex",gap:"0.4rem",fontSize:"0.92rem",color:hov?"rgba(255,255,255,0.7)":"var(--muted)",marginBottom:"0.35rem",lineHeight:1.6,fontFamily:"'DM Sans', system-ui, sans-serif"}}>
            <span style={{color:hov?"#A78BFA":cc,flexShrink:0,lineHeight:"1.7"}}>▸</span>{h}
          </div>
        ))}
      </div>
      <div style={{display:"flex",flexWrap:"wrap",gap:"0.3rem",marginBottom:"0.8rem"}}>
        {p.tech.map(t=>(
          <span key={t} style={{fontFamily:"'DM Sans', system-ui, sans-serif",fontSize:"0.85rem",fontWeight:500,padding:"0.28rem 0.65rem",background:hov?"rgba(167,139,250,0.15)":"var(--vPale)",color:hov?"#A78BFA":cc,border:`1px solid ${hov?"rgba(167,139,250,0.3)":cc}`}}>{t}</span>
        ))}
      </div>
      {(p.youtube||p.webapp||p.website||p.discord)&&(
        <div style={{display:"flex",flexWrap:"wrap",gap:"0.4rem",paddingTop:"0.8rem",borderTop:`2px solid ${hov?"rgba(255,255,255,0.08)":"rgba(124,58,237,0.1)"}`}}>
          ...
          {p.webapp&&<a href={p.webapp} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()} className="px-btn" style={{flex:1,minWidth:"80px",textAlign:"center",padding:"0.4rem 0.6rem",background:"#7c3aed",color:"#fff",textDecoration:"none",fontSize:"0.88rem",fontWeight:600,fontFamily:"'DM Sans', system-ui, sans-serif",border:"2px solid #4c1d95",boxShadow:"2px 2px 0 #4c1d95"}}>Live</a>}
...
function SkillBar({ name, pct, cat }) {
  ...
  return (
    <div ref={ref} style={{marginBottom:"1.1rem"}}>
      ...
      <div style={{height:10,background:"rgba(124,58,237,0.07)",border:"2px solid rgba(124,58,237,0.18)"}}>
    ...
    <section id="skills" style={{padding:"8rem 4rem",background:"var(--bg)",position:"relative",overflow:"hidden"}} className="sec-pad">
      ...
      <div style={{maxWidth:1060,margin:"0 auto",position:"relative",zIndex:1}}>
        ...
        <div className="rv d3" style={{display:"flex",gap:"0.4rem",marginBottom:"2rem",flexWrap:"wrap"}}>
          {TABS.map(([k,l])=>(
            <button key={k} onClick={()=>setTab(k)} style={{padding:"0.4rem 1rem",cursor:"pointer",border:"2px solid #7C3AED",background:tab===k?"#7C3AED":"transparent",color:tab===k?"#fff":"#7C3AED",fontSize:"0.92rem",fontWeight:600,fontFamily:"'DM Sans', system-ui, sans-serif",boxShadow:tab===k?"3px 3px 0 #4C1D95":"2px 2px 0 #4C1D95",transition:"all 0.1s"}}>{l}</button>
          ))}
        </div>
        {tab==="bars"&&(
      ...
      <div style={{maxWidth:1060,margin:"0 auto"}}>
        ...
        <div className="rv d5" style={{marginTop:"1.5rem",background:"#1a0a2e",border:"3px solid #7C3AED",padding:"2rem",display:"flex",alignItems:"center",gap:"3rem",flexWrap:"wrap",boxShadow:"6px 6px 0 #7C3AED"}}>
          <div>
            ...
            <div style={{fontFamily:"'DM Mono', monospace",fontSize:"2.5rem",color:"#fff",letterSpacing:"-1px",lineHeight:1,fontWeight:500}}>TOP <span style={{color:"#A78BFA"}}>0.3%</span></div>
          ...
          <div style={{display:"flex",gap:"2rem",flexWrap:"wrap"}}>
            {[["900+","Problems"],["Top 0.3%","Global Rank"],["All levels","Easy–Med–Hard"]].map(([n,l])=>(
              <div key={l} style={{textAlign:"center"}}>
                <div style={{fontFamily:"'DM Mono', monospace",fontSize:"1.5rem",color:"#A78BFA",fontWeight:500}}>{n}</div>
  ...
  return (
    <section id="blog" className="sec-pad sec-transition" style={{padding:"8rem 4rem",background:"var(--bg2)"}}>
      <div style={{maxWidth:1060,margin:"0 auto"}}>
        ...
        <p className="rv d2" style={{fontSize:"1rem",color:"var(--inkMid)",marginBottom:"2.5rem",lineHeight:1.8,fontFamily:"'DM Sans', system-ui, sans-serif"}}>Writing about what I build — problems, design decisions, learnings.</p>
          ...
          {BLOGS.map((b,i)=>(
            ...
            >
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1rem"}}>
                <span style={{fontFamily:"'DM Mono', monospace",fontSize:"0.88rem",color:"var(--muted)",fontWeight:500}}>#{b.num}</span>
                <div style={{display:"flex",gap:"0.5rem",alignItems:"center"}}>
                  <span style={{fontFamily:"'DM Sans', system-ui, sans-serif",fontSize:"0.88rem",color:"var(--inkMid)",fontWeight:500}}>{b.date}</span>
                  <span style={{width:4,height:4,background:"var(--muted)",display:"inline-block"}}/>
                  <span style={{fontFamily:"'DM Mono', monospace",fontSize:"0.85rem",color:b.col,fontWeight:500}}>{b.readTime}</span>
                </div>
              </div>
              <h3 style={{fontFamily:"'DM Sans', system-ui, sans-serif",fontSize:"1.05rem",fontWeight:600,color:"var(--ink)",lineHeight:1.55,marginBottom:"0.7rem"}}>{b.title}</h3>
              <p style={{fontSize:"0.97rem",color:"var(--muted)",lineHeight:1.8,marginBottom:"1rem",fontFamily:"'DM Sans', system-ui, sans-serif"}}>{b.excerpt}</p>
              <div style={{display:"flex",flexWrap:"wrap",gap:"0.3rem",marginBottom:"1rem"}}>
                {b.tags.map(t=><span key={t} className="chip" style={{fontSize:"0.82rem",borderColor:b.col,color:b.col,boxShadow:`2px 2px 0 ${b.col}`}}>{t}</span>)}
              </div>
              <div style={{display:"flex",alignItems:"center",gap:"0.3rem",fontFamily:"'DM Sans', system-ui, sans-serif",fontSize:"0.95rem",fontWeight:600,color:b.col}}>
                Read more →
              </div>
            </a>
          ))}
      ...
      <div style={{maxWidth:1060,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",flexWrap:"wrap",gap:"1rem",marginBottom:"0.6rem"}}>
          ...
          <div className="rv d2" style={{display:"flex",alignItems:"center",gap:"0.4rem",padding:"0.35rem 0.8rem",background:"var(--vPale)",border:"2px solid #7C3AED",boxShadow:"2px 2px 0 #7C3AED",animation:"learnPulse 2.5s ease-in-out infinite"}}>
            ...
            <span style={{fontFamily:"'DM Sans', system-ui, sans-serif",fontSize:"0.88rem",color:"#7C3AED",fontWeight:600}}>In progress</span>
          </div>
        </div>
        ...
        <div className="rv d5" style={{marginTop:"2rem",padding:"1rem 1.5rem",background:"var(--vPale)",border:"2px solid #7C3AED",boxShadow:"3px 3px 0 #7C3AED",display:"flex",alignItems:"center",gap:"0.8rem"}}>
          ...
          <span style={{fontFamily:"'DM Sans', system-ui, sans-serif",fontSize:"1rem",color:"var(--ink)",lineHeight:1.65}}>
            <strong style={{fontFamily:"'DM Sans', system-ui, sans-serif",fontSize:"1rem",color:"#7C3AED",fontWeight:600}}>Goal 2025–26:</strong> Ship prod-grade services in Go, get comfortable with K8s, contribute to open-source.
          </span>
        </div>
      </div>
    </section>
...
function Contact() {
  ...
  const copy=()=>{ navigator.clipboard.writeText("armanphaugat20@gmail.com"); setCopied(true); ChipTune.coin(); setTimeout(()=>setCopied(false),2000); };
  const LINKS=[
  ...
  return (
    <section id="contact" style={{padding:"9rem 4rem",background:"#05000c",position:"relative",overflow:"hidden",backgroundImage:"radial-gradient(ellipse 70% 60% at 50% 0%, rgba(124,58,237,0.15) 0%, transparent 60%)"}}>
      ...
      <div style={{maxWidth:1060,margin:"0 auto",position:"relative",zIndex:1}}>
        <div className="rv" style={{fontFamily:"'Press Start 2P', monospace",fontSize:"0.75rem",color:"#A78BFA",letterSpacing:"2px",marginBottom:"1rem",display:"flex",alignItems:"center",gap:"0.6rem"}}>
        ...
        <h2 className="rv d1" style={{fontFamily:"'Press Start 2P', monospace",fontSize:"clamp(1.5rem,4vw,3rem)",fontWeight:400,color:"#fff",marginBottom:"0.8rem",lineHeight:1.3}}>
          LET'S <span style={{color:"#A78BFA"}}>CONNECT</span>
        </h2>
        ...
        <div className="rv d3 contact-email-row" style={{display:"flex",gap:"0.8rem",alignItems:"center",marginBottom:"2.5rem",flexWrap:"wrap"}}>
          ...
          <button onClick={copy} className="px-btn" style={{padding:"0.9rem 1.5rem",background:copied?"#10B981":"#7C3AED",color:"#fff",border:"none",cursor:"pointer",fontSize:"0.95rem",fontWeight:600,fontFamily:"'DM Sans', system-ui, sans-serif",transition:"background 0.2s",boxShadow:"3px 3px 0 #4C1D95",whiteSpace:"nowrap"}}>{copied?"Copied! ✓":"Copy email"}</button>
  ...
  return (
    <footer style={{background:"linear-gradient(180deg,#050008 0%,#020005 100%)",borderTop:"3px solid #7C3AED",boxShadow:"0 -4px 30px rgba(124,58,237,0.15)"}}>
      ...
      <div style={{borderTop:"2px solid rgba(255,255,255,0.05)",padding:"1rem 2rem",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:"0.4rem",maxWidth:1060,margin:"0 auto"}}>
        ...
        <span style={{fontFamily:"'DM Mono', monospace",fontSize:"0.85rem",color:"rgba(255,255,255,0.25)"}}>Built with React · <span style={{color:"rgba(124,58,237,0.6)"}}>?</span> = shortcuts · <span style={{color:"rgba(124,58,237,0.6)"}}>`</span> = terminal</span>
      </div>
    </footer>
  );
}
